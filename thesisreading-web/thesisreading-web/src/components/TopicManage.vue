    ...
    <div id="topic-manage">
      ...
      <div style="float:right; margin-bottom:8px; margin-right:10px;">
        <el-button
          ...
          :circle="true"
          type="mini"
          @click="function(){addTopicVisible=true;addTopic()}"
        ></el-button>
      </div>

      <!-- Form  增加一个主题-->
      <el-dialog title="新增主题" :visible.sync="addTopicVisible">
        <el-form :model="addTopicForm" :rules="addTopicRules" ref="addTopicForm">
          <el-form-item prop="topicId" label="主题号" :label-width="formLabelWidth">
            <el-input v-model="addTopicForm.topicId" placeholder="请输入主题号(4位数字)"></el-input>
          </el-form-item>
          <el-form-item label="课程号" :label-width="formLabelWidth">
            <el-input v-model="addTopicForm.courseId" readonly></el-input>
          </el-form-item>

          <el-form-item label="课程" :label-width="formLabelWidth" prop="courseId">
            <el-select v-model="addTopicForm.courseId" placeholder="请选择所授课程">
              <el-option
          ...
          <el-form-item prop="topic" label="主题名">
            <el-input v-model="addTopicForm.topic" placeholder="请输入主题名"></el-input>
          </el-form-item>
          <el-form-item prop="keyword" label="关键字">
            <el-input v-model="addTopicForm.keyword" placeholder="请输入关键字"></el-input>
          </el-form-item>
          <el-form-item prop="maxNum" label="人数上限">
            <el-input v-model="addTopicForm.maxNum" placeholder="请输入人数上限"></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="function(){addTopicVisible = false; cancelAddTopic();}">取 消</el-button>
          <el-button type="primary" @click="submitAddTopic('addTopicForm');">确 定</el-button>
        </div>
      </el-dialog>
    </div>
      ...
      axios
        ...
        .then(success => {
          if (success.data.statusResp.code == "1") {
            this.$router.push({ path: `/teacher/topic/${row.topicId}/${this.courseId}` });
          } else {
            this.$message({
              type: "error",
